...
export class IssuesService {
  ...
  findAllMeetingIssues() {
    const query = this.issueModel.find();
    query.where('type', IssueType.MEETING);
    return query.exec();
  }
  ...
  async deleteIssuesByProjectIds(projectIds: string[], session: ClientSession) {
    // try {
    ...
    try {
      ...
      if (result.ok !== 1) {
        throw new InternalServerErrorException(
          `Error to delete the issues associated to the project`,
        );
      }
    } catch (error) {
      throw error;
  ...
  findAllUpcommingIssues() {
    ...
    const query = this.issueModel.find();
    // query.where('deadline').gte(today.getTime()).lt(tomorrow.getTime());
    return query.exec();
  }
}
