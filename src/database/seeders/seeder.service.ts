...
import { WorkspaceSeeder } from './workspaces/workspace-seeder.service';
import { TodoSeeder } from './todo/todo-seeder.service';
import { NoteSeeder } from './notes/note-seeder.service';
...
export class Seeder {
  ...
  constructor(
    ...
    private readonly todoSeeder: TodoSeeder,
    private readonly noteSeeder: NoteSeeder,
  ) {}
  ...
  private async seedCollections(session: ClientSession) {
    ...
    await this.workspaceSeeder.seed(session);
    await this.todoSeeder.seed(session);
    await this.noteSeeder.seed(session);
  }
}
